...
#define PROCESS_NAME "CONTROL_VER_VEL"
...
DataAccess<VerVelControl_params> params_access;

Control* ver_vel_control = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, VerVelControl_params& params);
void execute();
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, VerVelControl_params& params)
{
	...
	if (strcmp(logic_name, "control-ver-vel-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ver-vel-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-ver-vel-Kp-ref:") == 0)				sprintf(params.reference.kp.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ver-vel-Ki-ref:") == 0)				sprintf(params.reference.ki.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ver-vel-Kd-ref:") == 0)				sprintf(params.reference.kd.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "control-ver-vel-Kp-act:") == 0)				sprintf(params.actual.kp.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ver-vel-Ki-act:") == 0)				sprintf(params.actual.ki.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ver-vel-Kd-act:") == 0)				sprintf(params.actual.kd.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	ver_vel_control = new VerVelControl(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	ver_vel_control->create();
