...
#include "VerPosFilter.h"

#define PROCESS_NAME "FILTER_VERPOS"
...
DataAccess<VerPosFilter_params> params_access;

Filter* verpos_filter = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, VerPosFilter_params& params);
void execute();
...
int init_mqtt()
{
	...
	mosquitto_subscribe(mosq, NULL, params.reference.q_z.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.r_z.topic_name, 0);
	
	
	mosquitto_subscribe(mosq, NULL, ngc.pose.z.raw.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, ngc.velocityInertial.zDot.raw.topic_name, 0);
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, VerPosFilter_params& params)
{
	...
	if (strcmp(logic_name, "filter-ver-pos-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-pos-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "filter-ver-pos-q-z-ref:") == 0)				sprintf(params.reference.q_z.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-pos-r-z-ref:") == 0)				sprintf(params.reference.r_z.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "filter-ver-pos-q-z-act:") == 0)				sprintf(params.actual.q_z.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-pos-r-z-act:") == 0)				sprintf(params.actual.r_z.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	verpos_filter = new VerPosFilter(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	verpos_filter->create();
...
void on_message_recv_callback(struct mosquitto* mosq, void* obj, const struct mosquitto_message* msg)
{
	...
	checkMessage(params.reference.q_z, msg);		checkMessage(params.reference.r_z, msg);
	
	checkMessage(ngc.pose.z.raw, msg);
...
void execute()
{
	...
	publishMessage(params.actual.q_z);			publishMessage(params.actual.r_z);
	
	publishMessage(ngc.pose.z.filtered);					
