...
int init_mqtt()
{
	...
	mosquitto_subscribe(mosq, NULL, fog.rollRate.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, fog.pitchRate.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, fog.headingRate.topic_name, 0);
...
void on_message_recv_callback(struct mosquitto* mosq, void* obj, const struct mosquitto_message* msg)
{
	...
	{ ngc_var = &(ngc.pose.phi);  ngc_var->raw.copyValueFrom(fog.roll);  set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }

	if (checkMessage(gps_ahrs.roll, msg) == true && sensor.value == SENSOR_GPS_AHRS)
	{ ngc_var = &(ngc.pose.phi);  ngc_var->raw.copyValueFrom(gps_ahrs.roll); set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW);}


	if (checkMessage(fog.pitch, msg) == true && sensor.value == SENSOR_FOG)
	{ ngc_var = &(ngc.pose.theta);  ngc_var->raw.copyValueFrom(fog.pitch);  set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }

	if (checkMessage(gps_ahrs.pitch, msg) == true && sensor.value == SENSOR_GPS_AHRS)
	{ ngc_var = &(ngc.pose.theta);  ngc_var->raw.copyValueFrom(gps_ahrs.pitch); set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }


	if (checkMessage(fog.heading, msg) == true && sensor.value == SENSOR_FOG)
	{ ngc_var = &(ngc.pose.psi);  ngc_var->raw.copyValueFrom(fog.heading);  set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }


	if (checkMessage(gps_ahrs.heading, msg) == true && sensor.value == SENSOR_GPS_AHRS)
	{ ngc_var = &(ngc.pose.psi);  ngc_var->raw.copyValueFrom(gps_ahrs.heading);	set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }


	if (checkMessage(fog.rollRate, msg) == true && sensor.value == SENSOR_FOG)
	{ ngc_var = &(ngc.velocityBody.p);  ngc_var->raw.copyValueFrom(fog.rollRate);  set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }

	if (checkMessage(gps_ahrs.xGyro, msg) == true && sensor.value == SENSOR_GPS_AHRS)
	{ ngc_var = &(ngc.velocityBody.p);  ngc_var->raw.copyValueFrom(gps_ahrs.xGyro); set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }


	if (checkMessage(fog.pitchRate, msg) == true && sensor.value == SENSOR_FOG)
	{ ngc_var = &(ngc.velocityBody.q);  ngc_var->raw.copyValueFrom(fog.pitchRate);  set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }

	if (checkMessage(gps_ahrs.yGyro, msg) == true && sensor.value == SENSOR_GPS_AHRS)
	{ ngc_var = &(ngc.velocityBody.q);  ngc_var->raw.copyValueFrom(gps_ahrs.yGyro);	set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }


	if (checkMessage(fog.headingRate, msg) == true && sensor.value == SENSOR_FOG)
	{ ngc_var = &(ngc.velocityBody.r);  ngc_var->raw.copyValueFrom(fog.headingRate);  set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }

	if (checkMessage(gps_ahrs.zGyro, msg) == true && sensor.value == SENSOR_GPS_AHRS)
	{ ngc_var = &(ngc.velocityBody.r);  ngc_var->raw.copyValueFrom(gps_ahrs.zGyro); set_var(ngc_var->raw, ngc_var->actual, MEASURE_RAW); }
