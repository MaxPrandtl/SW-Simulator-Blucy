...
#define PROCESS_NAME "FILTER_HORPOS"
...
DataAccess<HorPosFilter_params> params_access;

Filter* horpos_filter = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, HorPosFilter_params& params);
void execute();
...
int init_mqtt()
{
	...
	mosquitto_subscribe(mosq, NULL, params.reference.q_xy.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.r_xy_11.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.r_xy_12.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.r_xy_22.topic_name, 0);

	mosquitto_subscribe(mosq, NULL, ngc.pose.x.raw.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, ngc.pose.y.raw.topic_name, 0);
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, HorPosFilter_params& params)
{
	...
	if (strcmp(logic_name, "filter-hor-pos-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "filter-hor-pos-q-xy-ref:") == 0)			sprintf(params.reference.q_xy.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-r-xy-11-ref:") == 0)			sprintf(params.reference.r_xy_11.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-r-xy-12-ref:") == 0)			sprintf(params.reference.r_xy_12.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-r-xy-22-ref:") == 0)			sprintf(params.reference.r_xy_22.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "filter-hor-pos-q-xy-act:") == 0)			sprintf(params.actual.q_xy.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-r-xy-11-act:") == 0)			sprintf(params.actual.r_xy_11.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-r-xy-12-act:") == 0)			sprintf(params.actual.r_xy_12.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-hor-pos-r-xy-22-act:") == 0)			sprintf(params.actual.r_xy_22.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	horpos_filter = new HorPosFilter(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	horpos_filter->create();
...
void on_message_recv_callback(struct mosquitto* mosq, void* obj, const struct mosquitto_message* msg)
{
	...
	checkMessage(params.reference.q_xy, msg);
	checkMessage(params.reference.r_xy_11, msg);	checkMessage(params.reference.r_xy_12, msg);	checkMessage(params.reference.r_xy_22, msg);
	
	checkMessage(ngc.pose.x.raw, msg);			checkMessage(ngc.pose.y.raw, msg);
...
void execute()
{
	...
	publishMessage(params.actual.q_xy);
	publishMessage(params.actual.r_xy_11);
	publishMessage(params.actual.r_xy_12);
	publishMessage(params.actual.r_xy_22);

	publishMessage(ngc.pose.x.filtered);
	publishMessage(ngc.pose.y.filtered);
