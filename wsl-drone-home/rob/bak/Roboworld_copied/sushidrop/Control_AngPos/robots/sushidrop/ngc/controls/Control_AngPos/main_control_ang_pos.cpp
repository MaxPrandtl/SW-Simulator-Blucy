...
#define PROCESS_NAME "CONTROL_ANG_POS"
...
DataAccess<AngPosControl_params> params_access;

Control* ang_pos_control = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, AngPosControl_params& params);
void execute();
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, AngPosControl_params& params)
{
	...
	if (strcmp(logic_name, "control-ang-pos-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-ang-vel-execution-status:") == 0)	sprintf(params.vel_control.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-ang-pos-Kp-vel-ref:") == 0)			sprintf(params.reference.kp_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Ki-vel-ref:") == 0)			sprintf(params.reference.ki_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Kd-vel-ref:") == 0)			sprintf(params.reference.kd_vel.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-ang-pos-Kp-vel-act:") == 0)			sprintf(params.actual.kp_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Ki-vel-act:") == 0)			sprintf(params.actual.ki_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Kd-vel-act:") == 0)			sprintf(params.actual.kd_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "control-ang-pos-Kp-force-ref:") == 0)		sprintf(params.reference.kp_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Ki-force-ref:") == 0)		sprintf(params.reference.ki_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Kd-force-ref:") == 0)		sprintf(params.reference.kd_force.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-ang-pos-Kp-force-act:") == 0)		sprintf(params.actual.kp_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Ki-force-act:") == 0)		sprintf(params.actual.ki_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-pos-Kd-force-act:") == 0)		sprintf(params.actual.kd_force.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	ang_pos_control = new AngPosControl(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	ang_pos_control->create();
...
void execute()
{
	...
	publishMessage(ngc.velocityBody.r.automatic);
	publishMessage(ngc.force.tr.automatic);
