...
AltControl::AltControl(const char* name, DataAccess<NGC_status>* NGC_access, DataAccess<Time_status>* Time_access, DataAccess<Task_execution>* Task_access, DataAccess<AltControl_params>* Params_access) :
	Control(name, ALT_CONTROL_PARAMS_FILE)
{
...
void AltControl::parse_config(char *value, double val)
{
	Time_status clock;
...
void AltControl::set_config(FILE* f)
{
...
{	
	...
	DoubleVariable aRef = ngc_status.pose.a.reference;

	aRef_prec = aRef.value;
}




void AltControl::compute()
{
	params = params_access->get();
	...
	DoubleVariable a = ngc_status.pose.a.actual;
	DoubleVariable aRef = ngc_status.pose.a.reference;
	...
	double e = aRef.value - a.value;
	ie += e * time_status.dt.value;
	double de = ((aRef.value - aRef_prec)/ time_status.dt.value) - w.value;
	aRef_prec = aRef.value;

	if (labs(time_status.timeStamp.value - aRef.timeStamp) <= 150)
		ie = 0.0;
	...
	{
		fw = -(kp_force * e + ki_force * ie + kd_force * de);
