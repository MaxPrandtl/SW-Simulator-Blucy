...
#define PROCESS_NAME "FILTER_VERVEL"
...
DataAccess<VerVelFilter_params> params_access;

Filter* vervel_filter = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, VerVelFilter_params& params);
void execute();
...
int init_mqtt()
{
	...
	mosquitto_subscribe(mosq, NULL, params.reference.q_w.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.r_w.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.Kw.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.bw.topic_name, 0);
	
	mosquitto_subscribe(mosq, NULL, ngc.velocityBody.w.raw.topic_name, 0);
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, VerVelFilter_params& params)
{
	...
	if (strcmp(logic_name, "filter-ver-vel-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "filter-ver-vel-q-w-ref:") == 0)				sprintf(params.reference.q_w.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-r-w-ref:") == 0)				sprintf(params.reference.r_w.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-Kw-ref:") == 0)				sprintf(params.reference.Kw.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-bw-ref:") == 0)				sprintf(params.reference.bw.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "filter-ver-vel-q-w-act:") == 0)				sprintf(params.actual.q_w.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-r-w-act:") == 0)				sprintf(params.actual.r_w.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-Kw-act:") == 0)				sprintf(params.actual.Kw.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "filter-ver-vel-bw-act:") == 0)				sprintf(params.actual.bw.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	vervel_filter = new VerVelFilter(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	vervel_filter->create();
...
void on_message_recv_callback(struct mosquitto* mosq, void* obj, const struct mosquitto_message* msg)
{
	...
	checkMessage(params.reference.q_w, msg);		checkMessage(params.reference.r_w, msg);
	checkMessage(params.reference.Kw, msg);			checkMessage(params.reference.bw, msg);
	
	checkMessage(ngc.pose.phi.actual, msg);			checkMessage(ngc.pose.theta.actual, msg);

	checkMessage(ngc.velocityBody.w.raw, msg);
...
void execute()
{
	...
	publishMessage(params.actual.q_w);			publishMessage(params.actual.r_w);
	publishMessage(params.actual.Kw);			publishMessage(params.actual.bw);

	publishMessage(ngc.velocityBody.w.filtered);					
	publishMessage(ngc.velocityInertial.zDot.filtered);
