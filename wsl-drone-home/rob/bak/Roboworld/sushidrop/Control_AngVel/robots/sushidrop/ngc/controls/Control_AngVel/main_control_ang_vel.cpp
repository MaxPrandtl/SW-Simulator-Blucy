...
#define PROCESS_NAME "CONTROL_ANG_VEL"
...
DataAccess<AngVelControl_params> params_access;

Control* ang_vel_control = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, AngVelControl_params& params);
void execute();
...
int init_mqtt()
{
	...
	mosquitto_subscribe(mosq, NULL, params.reference.kp.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.ki.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, params.reference.kd.topic_name, 0);
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, AngVelControl_params& params)
{
	...
	if (strcmp(logic_name, "control-ang-vel-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-vel-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-ang-vel-Kp-ref:") == 0)				sprintf(params.reference.kp.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-vel-Ki-ref:") == 0)				sprintf(params.reference.ki.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-vel-Kd-ref:") == 0)				sprintf(params.reference.kd.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "control-ang-vel-Kp-act:") == 0)				sprintf(params.actual.kp.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-vel-Ki-act:") == 0)				sprintf(params.actual.ki.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-ang-vel-Kd-act:") == 0)				sprintf(params.actual.kd.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	ang_vel_control = new AngVelControl(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	ang_vel_control->create();
...
void on_message_recv_callback(struct mosquitto* mosq, void* obj, const struct mosquitto_message* msg)
{
	...
	checkMessage(params.reference.kp, msg);
	checkMessage(params.reference.ki, msg);
	checkMessage(params.reference.kd, msg);
...
void execute()
{
	...
	publishMessage(params.actual.kp);
	publishMessage(params.actual.ki);
	publishMessage(params.actual.kd);
