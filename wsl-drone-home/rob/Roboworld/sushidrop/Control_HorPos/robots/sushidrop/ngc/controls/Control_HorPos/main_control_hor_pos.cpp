...
#define PROCESS_NAME "CONTROL_HOR_POS"
...
DataAccess<HorPosControl_params> params_access;

Control* hor_pos_control = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, HorPosControl_params& params);
void execute();
...
int init_mqtt()
{
	...
	mosquitto_subscribe(mosq, NULL, ngc.velocityBody.u.automatic.topic_name, 0);
	mosquitto_subscribe(mosq, NULL, ngc.velocityBody.v.automatic.topic_name, 0);
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, HorPosControl_params& params)
{
	...
	if (strcmp(logic_name, "control-hor-pos-execution-command:") == 0)	sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-execution-status:") == 0)	sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-hor-vel-execution-status:") == 0)	sprintf(params.vel_control.status.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-hor-pos-Kp-vel-ref:") == 0)			sprintf(params.reference.kp_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Ki-vel-ref:") == 0)			sprintf(params.reference.ki_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Kd-vel-ref:") == 0)			sprintf(params.reference.kd_vel.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-hor-pos-Kp-vel-act:") == 0)			sprintf(params.actual.kp_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Ki-vel-act:") == 0)			sprintf(params.actual.ki_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Kd-vel-act:") == 0)			sprintf(params.actual.kd_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "control-hor-pos-Kp-force-ref:") == 0)		sprintf(params.reference.kp_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Ki-force-ref:") == 0)		sprintf(params.reference.ki_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Kd-force-ref:") == 0)		sprintf(params.reference.kd_force.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-hor-pos-Kp-force-act:") == 0)		sprintf(params.actual.kp_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Ki-force-act:") == 0)		sprintf(params.actual.ki_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-hor-pos-Kd-force-act:") == 0)		sprintf(params.actual.kd_force.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	hor_pos_control = new HorPosControl(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	hor_pos_control->create();
...
void execute()
{
	...
	publishMessage(ngc.velocityBody.u.automatic);
	publishMessage(ngc.velocityBody.v.automatic);
