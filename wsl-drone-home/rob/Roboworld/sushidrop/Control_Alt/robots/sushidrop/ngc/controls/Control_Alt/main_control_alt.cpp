...
#include "AltControl.h"

#define PROCESS_NAME "CONTROL_ALT"
...
DataAccess<AltControl_params> params_access;

Control* alt_control = NULL;


void parse_topic_file(char* logic_name, char* topic_name, NGC_status &ngc, Time_status &clock, Task_execution& task, AltControl_params& params);
void execute();
...
void parse_topic_file(char* logic_name, char* topic_name, NGC_status& ngc, Time_status& clock, Task_execution& task, AltControl_params& params)
{
	...
	if (strcmp(logic_name, "control-alt-execution-command:") == 0)		sprintf(task.command.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-execution-status:") == 0)		sprintf(task.status.topic_name, "%s%s%s", environment, robot, topic_name);
	...
	if (strcmp(logic_name, "control-alt-Kp-vel-ref:") == 0)				sprintf(params.reference.kp_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Ki-vel-ref:") == 0)				sprintf(params.reference.ki_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Kd-vel-ref:") == 0)				sprintf(params.reference.kd_vel.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-alt-Kp-vel-act:") == 0)				sprintf(params.actual.kp_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Ki-vel-act:") == 0)				sprintf(params.actual.ki_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Kd-vel-act:") == 0)				sprintf(params.actual.kd_vel.topic_name, "%s%s%s", environment, robot, topic_name);
	
	if (strcmp(logic_name, "control-alt-Kp-force-ref:") == 0)			sprintf(params.reference.kp_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Ki-force-ref:") == 0)			sprintf(params.reference.ki_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Kd-force-ref:") == 0)			sprintf(params.reference.kd_force.topic_name, "%s%s%s", environment, robot, topic_name);

	if (strcmp(logic_name, "control-alt-Kp-force-act:") == 0)			sprintf(params.actual.kp_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Ki-force-act:") == 0)			sprintf(params.actual.ki_force.topic_name, "%s%s%s", environment, robot, topic_name);
	if (strcmp(logic_name, "control-alt-Kd-force-act:") == 0)			sprintf(params.actual.kd_force.topic_name, "%s%s%s", environment, robot, topic_name);
...
int main(int argc, char* argv[])
{
	...
	alt_control = new AltControl(PROCESS_NAME, &ngc_access, &time_access, &task_access,&params_access);
	alt_control->create();
