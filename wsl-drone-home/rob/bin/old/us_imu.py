...
class MSCL2MQTT():
    def __init__(self, mqtt_topic="us_imu", COM_PORT = "/dev/ttyS0", bitrate = 115200):
            ...
            while (True):
                ...
                for packet in packets:
                    ...
                    for dataPoint in packet.data():
                        ...
                        tpc = dataPoint.channelName()
                        self.current[tpc] = {"value": dataPoint.as_string(), "quality": "1"}
                        #if the dataPoint is invalid
                        if(dataPoint.valid() == False):
                            self.current[tpc]["quality"]="0"
                        topic = f"{self.mqtt_topic}"
                        if tpc.startswith("est"):
                            topic += "/estimator"
                        elif tpc.startswith("svi_"):
                            topic += "/svi"
                         
                        topic += "/"+tpc

                        mqtt.publish(topic, self.to_mqtt(self.current[tpc]))
                        self.broadcast_nmea()
    ...
    def to_mqtt(self, data):
        #print(data)
        return f"{data['value']} 0 {data['quality']}"
    ...
    def gen_gga(self, data):
        ...
        fix_quality = int(data['longitude']['quality'])*4 #Porcata per gestire la qualità del servizio. 0 => nessun dato 1 (dati corretti)=> 4 con fixed data
